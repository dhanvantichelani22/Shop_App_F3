...
form.addEventListener('submit',(event)=>{
    ...
    if(!firstName || !lastName || !email || !pass || !confPass){
        ...
        document.getElementById('message-error').innerText='Error :All Fields are Mandatory.'
        return;
    }
    ...
    if(pass!=confPass){
        document.getElementById('message').style.display='inline';
        document.getElementById('message').setAttribute('class','red')
        document.getElementById('message-error').innerText='Error :Password and Confirm Password Should be same.'
        return;
    }
    ...
    if(localStorage.getItem('totalUser')){
        ...
        totalUser.forEach((user)=>{
            if(user.email==email){
                ...
                document.getElementById('message').style.display='inline';
                // document.getElementById('message').setAttribute('class','red')
                document.getElementById('message-error').innerText='Error :User Already Exist.';
            }
        })
    }
    if(flag==true){
